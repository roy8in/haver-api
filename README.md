...
- **데이터 안정성**: 데이터 수집 중 오류가 발생할 경우 개별 재시도(Best-effort) 로직을 통해 데이터 누락을 방지합니다.
- **자동 테이블 생성**: 데이터 구조에 따라 DB 테이블을 자동으로 생성하고 Upsert(중복 시 업데이트)를 수행합니다.

## 프로젝트 구조
- `main.py`: 전체 수집 프로세스를 제어하는 실행 파일.
- `haver_provider.py`: Haver API 통신 및 데이터 전처리 전담.
- `db_handler.py`: 데이터베이스 API 통신 및 SQL 실행 전담.
- `tickers.csv`: 수집 대상 티커 목록 설정 파일.
- `.env`: API 키 및 URL 설정 파일 (보안상 공유 금지).
